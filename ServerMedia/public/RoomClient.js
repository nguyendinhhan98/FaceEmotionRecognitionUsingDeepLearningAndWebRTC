...
class RoomClient {
    ...
    async join(name, room_id) {
        ...
        socket.request('join', {
            ...
            listPersonInRoom = JSON.parse(e.peers);
    ...
    initSockets() {
        ...
        this.socket.on('newProducers', async function (data) {
            ...
            listPersonInRoom  = JSON.parse((await socket.request('getMyRoomInfo')).peers)
            console.log('listPersonInRoom', listPersonInRoom);
            for (let {
        ...
        this.socket.on('removeProducer', producer_id => {
    ...
    async produce(type, deviceId = null) {
        ...
        try {
            ...
            if (!audio) {
                ...
                elem.autoplay = true
                elem.className = "vid local-video"
    ...
    async consume(producer_id, producer_socket_id) {
        let name = 'unknown'
        console.log('producer_socket_id:', producer_socket_id)
        console.log('listPersonInRoom:',listPersonInRoom)
        if(listPersonInRoom.length > 0) {
            try {
                name = listPersonInRoom.find(ls => ls[0] === producer_socket_id)[1].name;
            } catch {
        ...
        this.getConsumeStream(producer_id).then(function ({
            ...
            if (kind === 'video') {
                ...
                elem.playsinline = false
                elem.autoplay = true
                elem.className = "vid remote-video"
                div.append(elem);
    ...
    removeConsumer(consumer_id) {
        ...
        if(elem) {
          elem.children[1].srcObject.getTracks().forEach(function (track) {
            track.stop()
          })
        elem.parentNode.removeChild(elem)
        }
